...
const EditCreative = () => {
    ...
    const fetchData = async (creativeId) => {
        try {
            const response = await adManagementApi.getCreativeDetails(creativeId, {
                includes: "lineItems",
                with_code: 1,
            });
            const {name, code, status, size, standard_code, id, lineItems} =
                response?.data;
            ...
            reset({
                ...
                standard_code,
                status_delivery,
                start_time_date: start_time? moment(start_time):null,
                start_time_hour: start_time?  moment(start_time).format("h:mm A"):null,
                end_time_date: end_time? moment(end_time):null,
                end_time_hour: end_time? moment(end_time).format("h:mm A"):null,
                goal_type,
                limit,
                event,
            });

            const isCheckedTime = start_time || end_time;
            setShowTime(isCheckedTime)
        } catch (err) {
            throw new Error(err);
        }
    };
    ...
    const onSubmit = async (data) => {
        ...
        try {
            ...
            const dataCreative = {name, status, size, standard_code};
            const dataLineItem = {
                ...
                event,
                status: status_delivery,
                creative_ids: [creativeId], // call api in step 2
            };

            // store creative and get creative_id
            const reponseCreative = await adManagementApi.updateCreative(
                creativeId,
                dataCreative
            );
            ...
            const reponseLineItem = await adManagementApi.updateLineItem(
                dataId?.lineItemId,
                dataLineItem
            );
            if (!reponseLineItem?.data) throw new Error();

            toast.success("Update creative success");
    ...
    return (
        <Box>
            <form onSubmit={handleSubmit(onSubmit)} style={{height: "100%"}}>
                {/* breadcrumb */}
                <Breadcrumb title="Edit Creative" items={BCrumb}>
                    <Button
                        color="primary"
                        variant="contained"
                        disableElevation
